...
class RotaryPositionEmbedding:
    ...
    Replaces absolute position embeddings with rotary encoding: applies a
    position-dependent rotation to query and key vectors so that attention
    scores naturally depend only on the relative distance between positions.

    No learnable parameters — cos/sin tables are precomputed once.
    """

    @staticmethod
    def precompute(max_seq_length, head_dim, base=10000.0, device="cpu"):
        """Precompute cos/sin tables for positions [0, max_seq_length).

        Returns:
            cos: [max_seq_length, head_dim]
            sin: [max_seq_length, head_dim]
        """
        inv_freq = 1.0 / (
            base
            ** (
        ...
        positions = torch.arange(max_seq_length, device=device).float()
    ...
    @staticmethod
    def apply(x, cos, sin):
        """Apply rotary position encoding to a single tensor (Q or K).

        Args:
            x:   [*, L, D] — query or key tensor
            cos: [*, L, D] — broadcastable to x
            sin: [*, L, D] — broadcastable to x
...
def compute_time_bias(timestamps, method="log", weight=1.0, mask_positions=None):
    """Compute time-interval attention bias from real timestamps.

    For each pair of positions (i, j), computes:
        bias[i,j] = -weight * f(|t_i - t_j|)
    where f is 'log1p' or 'linear'.  The bias is NEGATIVE so that items
    closer in time get higher attention scores (less penalty).

    Args:
        timestamps:     [B, L] float — UNIX timestamps for each position
        method:         'log' or 'linear' — time-interval transform
        weight:         scalar or learnable Parameter — scale factor
        mask_positions: [B, L] bool — True = masked (BERT4Rec); pairs
                        involving masked positions receive zero bias.

    Returns:
        bias: [B, 1, L, L] — additive attention bias (ADDED to scores before softmax)
    """
    B, L = timestamps.shape
    t1 = timestamps.unsqueeze(-1)  # [B, L, 1]
    t2 = timestamps.unsqueeze(1)  # [B, 1, L]
    delta = torch.abs(t1 - t2)  # [B, L, L] — raw seconds

    if method == "log":
        delta = torch.log1p(delta)  # log(1 + seconds)
    # else 'linear': use raw delta

    # Negative sign: closer in time → smaller delta → less negative bias → higher attn
    bias = -weight * delta  # [B, L, L]
    bias = bias.unsqueeze(1)  # [B, 1, L, L]

    # Zero out pairs involving masked positions (for BERT4RecP)
    if mask_positions is not None:
        qm = mask_positions.unsqueeze(1).unsqueeze(-1).float()  # [B, 1, L, 1]
        km = mask_positions.unsqueeze(1).unsqueeze(2).float()  # [B, 1, 1, L]
        bias = bias * (1 - qm) * (1 - km)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        rope_cos=None, rope_sin=None, time_bias=None,
    ):
        mixed_query_layer = self.query(input_tensor)
        mixed_key_layer = self.key(input_tensor)
        mixed_value_layer = self.value(input_tensor)

        # Q: [B, nH, L, D] — ready for RoPE and matmul
        query_layer = self.transpose_for_scores(mixed_query_layer).permute(0, 2, 1, 3)
        # K: [B, nH, L, D] — RoPE needs this shape; permute for matmul later
        ...
        if rope_cos is not None and rope_sin is not None:
            # cos/sin: [B, 1, L, D] for broadcast over heads
            cos = rope_cos.unsqueeze(1)
            sin = rope_sin.unsqueeze(1)
            query_layer = RotaryPositionEmbedding.apply(query_layer, cos, sin)
            key_layer = RotaryPositionEmbedding.apply(key_layer, cos, sin)

        # K: [B, nH, D, L] for matmul with Q [B, nH, L, D]
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, **kwargs):
        attention_output = self.multi_head_attention(hidden_states, attention_mask, **kwargs)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, **kwargs):
        """
        Args:
