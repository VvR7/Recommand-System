# -*- coding: utf-8 -*-
"""
BERT4RecP — BERT4Rec with three plug-and-play innovations:

1. RoPE (Rotary Position Embedding) — Su et al. 2021
   Replaces absolute position embeddings with rotary encoding.

2. Time Interval Attention Bias
   Adds a learnable bias to attention scores based on real timestamps.
   Masked (Cloze) positions are excluded from time-bias computation.

3. Feature-aware Contrastive Loss (InfoNCE)
   Auxiliary loss computed on the UNMASKED item sequence, pulling same-
   category items closer in embedding space.
...
from recbole.model.sequential_recommender.bert4recf import BERT4RecF
...
class BERT4RecP(BERT4RecF):
    """BERT4Rec with RoPE, TimeIntervalBias, and Contrastive auxiliary loss."""

    def __init__(self, config, dataset):
        # ── Let BERT4RecF set up item_emb, feature_embed_layer, feat_concat_layer,
        #    TrmEncoder, position_embedding, loss, etc. ──────────────────────
        ...
        if self.use_rope:
            ...
            self.register_buffer("rope_cos_table", cos)  # [L, head_dim]
    ...
    def forward(self, item_seq, timestamps=None):
        """Forward pass with optional RoPE and time-bias injection.

        Args:
            item_seq:   [B, L] — may contain MASK token (self.mask_token)
            timestamps: [B, L] or None — real timestamps for time bias

        Returns:
            output: [B, L, H]
        """
        # Mask tracking (same as BERT4RecF)
        mask_positions = (item_seq == self.mask_token)  # [B, L]
        feat_seq = item_seq.clone()
        feat_seq[mask_positions] = 0  # safe lookup

        # 1. Item + feature embeddings (same as BERT4RecF)
        ...
        if not self.use_rope:
            ...
            extra_kwargs["rope_cos"] = cos
            extra_kwargs["rope_sin"] = sin
            # RoPE IS applied to masked positions — same as absolute PE

        # 3. Time interval bias (masked positions excluded)
        ...
        # 4. Bidirectional Transformer + output head (same as BERT4RecF)
    ...
    def calculate_loss(self, interaction):
        ...
        # Timestamps: same shape as masked_item_seq (MASK tokens inserted
        # into original timestamps — but time bias excludes them anyway)
        timestamps = None
        if self.use_time_bias:
            if hasattr(self, "TIME_SEQ_FIELD") and self.TIME_SEQ_FIELD in interaction:
                timestamps = interaction[self.TIME_SEQ_FIELD]

        seq_output = self.forward(masked_item_seq, timestamps=timestamps)

        # ── Gather masked positions (same as BERT4Rec) ─────────────────
        ...
        # ── Contrastive auxiliary loss (on UNMASKED sequence) ──────────
        if self.use_contra_loss:
            unmasked_seq = interaction[self.ITEM_SEQ]  # original, no MASK tokens
            contra_loss = self._contrastive_loss(unmasked_seq)
            return main_loss, contra_loss

        return main_loss

    # ── Contrastive loss (same as SASRecP) ────────────────────────────────

    def _contrastive_loss(self, item_seq):
        """InfoNCE loss over item embeddings, pulling same-category items closer.

        Supports "binary" and "hierarchical" weighting.
        """
        items = torch.unique(item_seq[item_seq > 0])
        if items.size(0) < 2:
            return torch.tensor(0.0, device=item_seq.device, requires_grad=True)

        cats = self.item_category_ids[items]
        embs = self.item_embedding(items)
        embs = F.normalize(embs, dim=-1)
        sim = (embs @ embs.T) / self.contra_temperature
        ...
        if self.contra_weighting == "binary":
            ...
            pos_mask = torch.zeros(
                items.size(0), items.size(0), device=item_seq.device
            )
            pos_mask = pos_mask.masked_fill(same_l3 & not_self, 1.0)
            pos_mask = pos_mask.masked_fill(same_l2 & ~same_l3 & not_self, 0.5)
            pos_mask = pos_mask.masked_fill(same_l1 & ~same_l2 & not_self, 0.2)
